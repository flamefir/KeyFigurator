//! Keycode string <-> u16 translation for the Raw HID wire protocol.
//!
//! The app's model stores QMK keycode strings ("KC_A", "MO(1)", "HOST(0)");
//! the firmware speaks numeric 16-bit QMK keycodes. The KEYCODES table below
//! is AUTO-GENERATED from the firmware repo's own keycode spec so the two
//! sides cannot drift (regeneration command in the vault Firmware note).

use std::collections::HashMap;
use std::sync::OnceLock;

/// (canonical name or alias, 16-bit QMK keycode value)
pub const KEYCODES: [(&str, u16); 1353] = [
    ("KC_NO", 0x0000),
    ("XXXXXXX", 0x0000),
    ("KC_TRANSPARENT", 0x0001),
    ("_______", 0x0001),
    ("KC_TRNS", 0x0001),
    ("KC_A", 0x0004),
    ("KC_B", 0x0005),
    ("KC_C", 0x0006),
    ("KC_D", 0x0007),
    ("KC_E", 0x0008),
    ("KC_F", 0x0009),
    ("KC_G", 0x000A),
    ("KC_H", 0x000B),
    ("KC_I", 0x000C),
    ("KC_J", 0x000D),
    ("KC_K", 0x000E),
    ("KC_L", 0x000F),
    ("KC_M", 0x0010),
    ("KC_N", 0x0011),
    ("KC_O", 0x0012),
    ("KC_P", 0x0013),
    ("KC_Q", 0x0014),
    ("KC_R", 0x0015),
    ("KC_S", 0x0016),
    ("KC_T", 0x0017),
    ("KC_U", 0x0018),
    ("KC_V", 0x0019),
    ("KC_W", 0x001A),
    ("KC_X", 0x001B),
    ("KC_Y", 0x001C),
    ("KC_Z", 0x001D),
    ("KC_1", 0x001E),
    ("KC_2", 0x001F),
    ("KC_3", 0x0020),
    ("KC_4", 0x0021),
    ("KC_5", 0x0022),
    ("KC_6", 0x0023),
    ("KC_7", 0x0024),
    ("KC_8", 0x0025),
    ("KC_9", 0x0026),
    ("KC_0", 0x0027),
    ("KC_ENTER", 0x0028),
    ("KC_ENT", 0x0028),
    ("KC_ESCAPE", 0x0029),
    ("KC_ESC", 0x0029),
    ("KC_BACKSPACE", 0x002A),
    ("KC_BSPC", 0x002A),
    ("KC_TAB", 0x002B),
    ("KC_SPACE", 0x002C),
    ("KC_SPC", 0x002C),
    ("KC_MINUS", 0x002D),
    ("KC_MINS", 0x002D),
    ("KC_EQUAL", 0x002E),
    ("KC_EQL", 0x002E),
    ("KC_LEFT_BRACKET", 0x002F),
    ("KC_LBRC", 0x002F),
    ("KC_RIGHT_BRACKET", 0x0030),
    ("KC_RBRC", 0x0030),
    ("KC_BACKSLASH", 0x0031),
    ("KC_BSLS", 0x0031),
    ("KC_NONUS_HASH", 0x0032),
    ("KC_NUHS", 0x0032),
    ("KC_SEMICOLON", 0x0033),
    ("KC_SCLN", 0x0033),
    ("KC_QUOTE", 0x0034),
    ("KC_QUOT", 0x0034),
    ("KC_GRAVE", 0x0035),
    ("KC_GRV", 0x0035),
    ("KC_COMMA", 0x0036),
    ("KC_COMM", 0x0036),
    ("KC_DOT", 0x0037),
    ("KC_SLASH", 0x0038),
    ("KC_SLSH", 0x0038),
    ("KC_CAPS_LOCK", 0x0039),
    ("KC_CAPS", 0x0039),
    ("KC_F1", 0x003A),
    ("KC_F2", 0x003B),
    ("KC_F3", 0x003C),
    ("KC_F4", 0x003D),
    ("KC_F5", 0x003E),
    ("KC_F6", 0x003F),
    ("KC_F7", 0x0040),
    ("KC_F8", 0x0041),
    ("KC_F9", 0x0042),
    ("KC_F10", 0x0043),
    ("KC_F11", 0x0044),
    ("KC_F12", 0x0045),
    ("KC_PRINT_SCREEN", 0x0046),
    ("KC_PSCR", 0x0046),
    ("KC_SCROLL_LOCK", 0x0047),
    ("KC_SCRL", 0x0047),
    ("KC_BRMD", 0x0047),
    ("KC_PAUSE", 0x0048),
    ("KC_PAUS", 0x0048),
    ("KC_BRK", 0x0048),
    ("KC_BRMU", 0x0048),
    ("KC_INSERT", 0x0049),
    ("KC_INS", 0x0049),
    ("KC_HOME", 0x004A),
    ("KC_PAGE_UP", 0x004B),
    ("KC_PGUP", 0x004B),
    ("KC_DELETE", 0x004C),
    ("KC_DEL", 0x004C),
    ("KC_END", 0x004D),
    ("KC_PAGE_DOWN", 0x004E),
    ("KC_PGDN", 0x004E),
    ("KC_RIGHT", 0x004F),
    ("KC_RGHT", 0x004F),
    ("KC_LEFT", 0x0050),
    ("KC_DOWN", 0x0051),
    ("KC_UP", 0x0052),
    ("KC_NUM_LOCK", 0x0053),
    ("KC_NUM", 0x0053),
    ("KC_KP_SLASH", 0x0054),
    ("KC_PSLS", 0x0054),
    ("KC_KP_ASTERISK", 0x0055),
    ("KC_PAST", 0x0055),
    ("KC_KP_MINUS", 0x0056),
    ("KC_PMNS", 0x0056),
    ("KC_KP_PLUS", 0x0057),
    ("KC_PPLS", 0x0057),
    ("KC_KP_ENTER", 0x0058),
    ("KC_PENT", 0x0058),
    ("KC_KP_1", 0x0059),
    ("KC_P1", 0x0059),
    ("KC_KP_2", 0x005A),
    ("KC_P2", 0x005A),
    ("KC_KP_3", 0x005B),
    ("KC_P3", 0x005B),
    ("KC_KP_4", 0x005C),
    ("KC_P4", 0x005C),
    ("KC_KP_5", 0x005D),
    ("KC_P5", 0x005D),
    ("KC_KP_6", 0x005E),
    ("KC_P6", 0x005E),
    ("KC_KP_7", 0x005F),
    ("KC_P7", 0x005F),
    ("KC_KP_8", 0x0060),
    ("KC_P8", 0x0060),
    ("KC_KP_9", 0x0061),
    ("KC_P9", 0x0061),
    ("KC_KP_0", 0x0062),
    ("KC_P0", 0x0062),
    ("KC_KP_DOT", 0x0063),
    ("KC_PDOT", 0x0063),
    ("KC_NONUS_BACKSLASH", 0x0064),
    ("KC_NUBS", 0x0064),
    ("KC_APPLICATION", 0x0065),
    ("KC_APP", 0x0065),
    ("KC_KB_POWER", 0x0066),
    ("KC_KP_EQUAL", 0x0067),
    ("KC_PEQL", 0x0067),
    ("KC_F13", 0x0068),
    ("KC_F14", 0x0069),
    ("KC_F15", 0x006A),
    ("KC_F16", 0x006B),
    ("KC_F17", 0x006C),
    ("KC_F18", 0x006D),
    ("KC_F19", 0x006E),
    ("KC_F20", 0x006F),
    ("KC_F21", 0x0070),
    ("KC_F22", 0x0071),
    ("KC_F23", 0x0072),
    ("KC_F24", 0x0073),
    ("KC_EXECUTE", 0x0074),
    ("KC_EXEC", 0x0074),
    ("KC_HELP", 0x0075),
    ("KC_MENU", 0x0076),
    ("KC_SELECT", 0x0077),
    ("KC_SLCT", 0x0077),
    ("KC_STOP", 0x0078),
    ("KC_AGAIN", 0x0079),
    ("KC_AGIN", 0x0079),
    ("KC_UNDO", 0x007A),
    ("KC_CUT", 0x007B),
    ("KC_COPY", 0x007C),
    ("KC_PASTE", 0x007D),
    ("KC_PSTE", 0x007D),
    ("KC_FIND", 0x007E),
    ("KC_KB_MUTE", 0x007F),
    ("KC_KB_VOLUME_UP", 0x0080),
    ("KC_KB_VOLUME_DOWN", 0x0081),
    ("KC_LOCKING_CAPS_LOCK", 0x0082),
    ("KC_LCAP", 0x0082),
    ("KC_LOCKING_NUM_LOCK", 0x0083),
    ("KC_LNUM", 0x0083),
    ("KC_LOCKING_SCROLL_LOCK", 0x0084),
    ("KC_LSCR", 0x0084),
    ("KC_KP_COMMA", 0x0085),
    ("KC_PCMM", 0x0085),
    ("KC_KP_EQUAL_AS400", 0x0086),
    ("KC_INTERNATIONAL_1", 0x0087),
    ("KC_INT1", 0x0087),
    ("KC_INTERNATIONAL_2", 0x0088),
    ("KC_INT2", 0x0088),
    ("KC_INTERNATIONAL_3", 0x0089),
    ("KC_INT3", 0x0089),
    ("KC_INTERNATIONAL_4", 0x008A),
    ("KC_INT4", 0x008A),
    ("KC_INTERNATIONAL_5", 0x008B),
    ("KC_INT5", 0x008B),
    ("KC_INTERNATIONAL_6", 0x008C),
    ("KC_INT6", 0x008C),
    ("KC_INTERNATIONAL_7", 0x008D),
    ("KC_INT7", 0x008D),
    ("KC_INTERNATIONAL_8", 0x008E),
    ("KC_INT8", 0x008E),
    ("KC_INTERNATIONAL_9", 0x008F),
    ("KC_INT9", 0x008F),
    ("KC_LANGUAGE_1", 0x0090),
    ("KC_LNG1", 0x0090),
    ("KC_LANGUAGE_2", 0x0091),
    ("KC_LNG2", 0x0091),
    ("KC_LANGUAGE_3", 0x0092),
    ("KC_LNG3", 0x0092),
    ("KC_LANGUAGE_4", 0x0093),
    ("KC_LNG4", 0x0093),
    ("KC_LANGUAGE_5", 0x0094),
    ("KC_LNG5", 0x0094),
    ("KC_LANGUAGE_6", 0x0095),
    ("KC_LNG6", 0x0095),
    ("KC_LANGUAGE_7", 0x0096),
    ("KC_LNG7", 0x0096),
    ("KC_LANGUAGE_8", 0x0097),
    ("KC_LNG8", 0x0097),
    ("KC_LANGUAGE_9", 0x0098),
    ("KC_LNG9", 0x0098),
    ("KC_ALTERNATE_ERASE", 0x0099),
    ("KC_ERAS", 0x0099),
    ("KC_SYSTEM_REQUEST", 0x009A),
    ("KC_SYRQ", 0x009A),
    ("KC_CANCEL", 0x009B),
    ("KC_CNCL", 0x009B),
    ("KC_CLEAR", 0x009C),
    ("KC_CLR", 0x009C),
    ("KC_PRIOR", 0x009D),
    ("KC_PRIR", 0x009D),
    ("KC_RETURN", 0x009E),
    ("KC_RETN", 0x009E),
    ("KC_SEPARATOR", 0x009F),
    ("KC_SEPR", 0x009F),
    ("KC_OUT", 0x00A0),
    ("KC_OPER", 0x00A1),
    ("KC_CLEAR_AGAIN", 0x00A2),
    ("KC_CLAG", 0x00A2),
    ("KC_CRSEL", 0x00A3),
    ("KC_CRSL", 0x00A3),
    ("KC_EXSEL", 0x00A4),
    ("KC_EXSL", 0x00A4),
    ("KC_SYSTEM_POWER", 0x00A5),
    ("KC_PWR", 0x00A5),
    ("KC_SYSTEM_SLEEP", 0x00A6),
    ("KC_SLEP", 0x00A6),
    ("KC_SYSTEM_WAKE", 0x00A7),
    ("KC_WAKE", 0x00A7),
    ("KC_AUDIO_MUTE", 0x00A8),
    ("KC_MUTE", 0x00A8),
    ("KC_AUDIO_VOL_UP", 0x00A9),
    ("KC_VOLU", 0x00A9),
    ("KC_AUDIO_VOL_DOWN", 0x00AA),
    ("KC_VOLD", 0x00AA),
    ("KC_MEDIA_NEXT_TRACK", 0x00AB),
    ("KC_MNXT", 0x00AB),
    ("KC_MEDIA_PREV_TRACK", 0x00AC),
    ("KC_MPRV", 0x00AC),
    ("KC_MEDIA_STOP", 0x00AD),
    ("KC_MSTP", 0x00AD),
    ("KC_MEDIA_PLAY_PAUSE", 0x00AE),
    ("KC_MPLY", 0x00AE),
    ("KC_MEDIA_SELECT", 0x00AF),
    ("KC_MSEL", 0x00AF),
    ("KC_MEDIA_EJECT", 0x00B0),
    ("KC_EJCT", 0x00B0),
    ("KC_MAIL", 0x00B1),
    ("KC_CALCULATOR", 0x00B2),
    ("KC_CALC", 0x00B2),
    ("KC_MY_COMPUTER", 0x00B3),
    ("KC_MYCM", 0x00B3),
    ("KC_WWW_SEARCH", 0x00B4),
    ("KC_WSCH", 0x00B4),
    ("KC_WWW_HOME", 0x00B5),
    ("KC_WHOM", 0x00B5),
    ("KC_WWW_BACK", 0x00B6),
    ("KC_WBAK", 0x00B6),
    ("KC_WWW_FORWARD", 0x00B7),
    ("KC_WFWD", 0x00B7),
    ("KC_WWW_STOP", 0x00B8),
    ("KC_WSTP", 0x00B8),
    ("KC_WWW_REFRESH", 0x00B9),
    ("KC_WREF", 0x00B9),
    ("KC_WWW_FAVORITES", 0x00BA),
    ("KC_WFAV", 0x00BA),
    ("KC_MEDIA_FAST_FORWARD", 0x00BB),
    ("KC_MFFD", 0x00BB),
    ("KC_MEDIA_REWIND", 0x00BC),
    ("KC_MRWD", 0x00BC),
    ("KC_BRIGHTNESS_UP", 0x00BD),
    ("KC_BRIU", 0x00BD),
    ("KC_BRIGHTNESS_DOWN", 0x00BE),
    ("KC_BRID", 0x00BE),
    ("KC_CONTROL_PANEL", 0x00BF),
    ("KC_CPNL", 0x00BF),
    ("KC_ASSISTANT", 0x00C0),
    ("KC_ASST", 0x00C0),
    ("KC_MISSION_CONTROL", 0x00C1),
    ("KC_MCTL", 0x00C1),
    ("KC_LAUNCHPAD", 0x00C2),
    ("KC_LPAD", 0x00C2),
    ("QK_MOUSE_CURSOR_UP", 0x00CD),
    ("MS_UP", 0x00CD),
    ("QK_MOUSE_CURSOR_DOWN", 0x00CE),
    ("MS_DOWN", 0x00CE),
    ("QK_MOUSE_CURSOR_LEFT", 0x00CF),
    ("MS_LEFT", 0x00CF),
    ("QK_MOUSE_CURSOR_RIGHT", 0x00D0),
    ("MS_RGHT", 0x00D0),
    ("QK_MOUSE_BUTTON_1", 0x00D1),
    ("MS_BTN1", 0x00D1),
    ("QK_MOUSE_BUTTON_2", 0x00D2),
    ("MS_BTN2", 0x00D2),
    ("QK_MOUSE_BUTTON_3", 0x00D3),
    ("MS_BTN3", 0x00D3),
    ("QK_MOUSE_BUTTON_4", 0x00D4),
    ("MS_BTN4", 0x00D4),
    ("QK_MOUSE_BUTTON_5", 0x00D5),
    ("MS_BTN5", 0x00D5),
    ("QK_MOUSE_BUTTON_6", 0x00D6),
    ("MS_BTN6", 0x00D6),
    ("QK_MOUSE_BUTTON_7", 0x00D7),
    ("MS_BTN7", 0x00D7),
    ("QK_MOUSE_BUTTON_8", 0x00D8),
    ("MS_BTN8", 0x00D8),
    ("QK_MOUSE_WHEEL_UP", 0x00D9),
    ("MS_WHLU", 0x00D9),
    ("QK_MOUSE_WHEEL_DOWN", 0x00DA),
    ("MS_WHLD", 0x00DA),
    ("QK_MOUSE_WHEEL_LEFT", 0x00DB),
    ("MS_WHLL", 0x00DB),
    ("QK_MOUSE_WHEEL_RIGHT", 0x00DC),
    ("MS_WHLR", 0x00DC),
    ("QK_MOUSE_ACCELERATION_0", 0x00DD),
    ("MS_ACL0", 0x00DD),
    ("QK_MOUSE_ACCELERATION_1", 0x00DE),
    ("MS_ACL1", 0x00DE),
    ("QK_MOUSE_ACCELERATION_2", 0x00DF),
    ("MS_ACL2", 0x00DF),
    ("KC_LEFT_CTRL", 0x00E0),
    ("KC_LCTL", 0x00E0),
    ("KC_LEFT_SHIFT", 0x00E1),
    ("KC_LSFT", 0x00E1),
    ("KC_LEFT_ALT", 0x00E2),
    ("KC_LALT", 0x00E2),
    ("KC_LOPT", 0x00E2),
    ("KC_LEFT_GUI", 0x00E3),
    ("KC_LGUI", 0x00E3),
    ("KC_LCMD", 0x00E3),
    ("KC_LWIN", 0x00E3),
    ("KC_RIGHT_CTRL", 0x00E4),
    ("KC_RCTL", 0x00E4),
    ("KC_RIGHT_SHIFT", 0x00E5),
    ("KC_RSFT", 0x00E5),
    ("KC_RIGHT_ALT", 0x00E6),
    ("KC_RALT", 0x00E6),
    ("KC_ROPT", 0x00E6),
    ("KC_ALGR", 0x00E6),
    ("KC_RIGHT_GUI", 0x00E7),
    ("KC_RGUI", 0x00E7),
    ("KC_RCMD", 0x00E7),
    ("KC_RWIN", 0x00E7),
    ("QK_SWAP_HANDS_TOGGLE", 0x56F0),
    ("SH_TOGG", 0x56F0),
    ("QK_SWAP_HANDS_TAP_TOGGLE", 0x56F1),
    ("SH_TT", 0x56F1),
    ("QK_SWAP_HANDS_MOMENTARY_ON", 0x56F2),
    ("SH_MON", 0x56F2),
    ("QK_SWAP_HANDS_MOMENTARY_OFF", 0x56F3),
    ("SH_MOFF", 0x56F3),
    ("QK_SWAP_HANDS_OFF", 0x56F4),
    ("SH_OFF", 0x56F4),
    ("QK_SWAP_HANDS_ON", 0x56F5),
    ("SH_ON", 0x56F5),
    ("QK_SWAP_HANDS_ONE_SHOT", 0x56F6),
    ("SH_OS", 0x56F6),
    ("QK_MAGIC_SWAP_CONTROL_CAPS_LOCK", 0x7000),
    ("CL_SWAP", 0x7000),
    ("QK_MAGIC_UNSWAP_CONTROL_CAPS_LOCK", 0x7001),
    ("CL_NORM", 0x7001),
    ("QK_MAGIC_TOGGLE_CONTROL_CAPS_LOCK", 0x7002),
    ("CL_TOGG", 0x7002),
    ("QK_MAGIC_CAPS_LOCK_AS_CONTROL_OFF", 0x7003),
    ("CL_CAPS", 0x7003),
    ("QK_MAGIC_CAPS_LOCK_AS_CONTROL_ON", 0x7004),
    ("CL_CTRL", 0x7004),
    ("QK_MAGIC_SWAP_LALT_LGUI", 0x7005),
    ("AG_LSWP", 0x7005),
    ("QK_MAGIC_UNSWAP_LALT_LGUI", 0x7006),
    ("AG_LNRM", 0x7006),
    ("QK_MAGIC_SWAP_RALT_RGUI", 0x7007),
    ("AG_RSWP", 0x7007),
    ("QK_MAGIC_UNSWAP_RALT_RGUI", 0x7008),
    ("AG_RNRM", 0x7008),
    ("QK_MAGIC_GUI_ON", 0x7009),
    ("GU_ON", 0x7009),
    ("QK_MAGIC_GUI_OFF", 0x700A),
    ("GU_OFF", 0x700A),
    ("QK_MAGIC_TOGGLE_GUI", 0x700B),
    ("GU_TOGG", 0x700B),
    ("QK_MAGIC_SWAP_GRAVE_ESC", 0x700C),
    ("GE_SWAP", 0x700C),
    ("QK_MAGIC_UNSWAP_GRAVE_ESC", 0x700D),
    ("GE_NORM", 0x700D),
    ("QK_MAGIC_SWAP_BACKSLASH_BACKSPACE", 0x700E),
    ("BS_SWAP", 0x700E),
    ("QK_MAGIC_UNSWAP_BACKSLASH_BACKSPACE", 0x700F),
    ("BS_NORM", 0x700F),
    ("QK_MAGIC_TOGGLE_BACKSLASH_BACKSPACE", 0x7010),
    ("BS_TOGG", 0x7010),
    ("QK_MAGIC_NKRO_ON", 0x7011),
    ("NK_ON", 0x7011),
    ("QK_MAGIC_NKRO_OFF", 0x7012),
    ("NK_OFF", 0x7012),
    ("QK_MAGIC_TOGGLE_NKRO", 0x7013),
    ("NK_TOGG", 0x7013),
    ("QK_MAGIC_SWAP_ALT_GUI", 0x7014),
    ("AG_SWAP", 0x7014),
    ("QK_MAGIC_UNSWAP_ALT_GUI", 0x7015),
    ("AG_NORM", 0x7015),
    ("QK_MAGIC_TOGGLE_ALT_GUI", 0x7016),
    ("AG_TOGG", 0x7016),
    ("QK_MAGIC_SWAP_LCTL_LGUI", 0x7017),
    ("CG_LSWP", 0x7017),
    ("QK_MAGIC_UNSWAP_LCTL_LGUI", 0x7018),
    ("CG_LNRM", 0x7018),
    ("QK_MAGIC_SWAP_RCTL_RGUI", 0x7019),
    ("CG_RSWP", 0x7019),
    ("QK_MAGIC_UNSWAP_RCTL_RGUI", 0x701A),
    ("CG_RNRM", 0x701A),
    ("QK_MAGIC_SWAP_CTL_GUI", 0x701B),
    ("CG_SWAP", 0x701B),
    ("QK_MAGIC_UNSWAP_CTL_GUI", 0x701C),
    ("CG_NORM", 0x701C),
    ("QK_MAGIC_TOGGLE_CTL_GUI", 0x701D),
    ("CG_TOGG", 0x701D),
    ("QK_MAGIC_EE_HANDS_LEFT", 0x701E),
    ("EH_LEFT", 0x701E),
    ("QK_MAGIC_EE_HANDS_RIGHT", 0x701F),
    ("EH_RGHT", 0x701F),
    ("QK_MAGIC_SWAP_ESCAPE_CAPS_LOCK", 0x7020),
    ("EC_SWAP", 0x7020),
    ("QK_MAGIC_UNSWAP_ESCAPE_CAPS_LOCK", 0x7021),
    ("EC_NORM", 0x7021),
    ("QK_MAGIC_TOGGLE_ESCAPE_CAPS_LOCK", 0x7022),
    ("EC_TOGG", 0x7022),
    ("QK_MIDI_ON", 0x7100),
    ("MI_ON", 0x7100),
    ("QK_MIDI_OFF", 0x7101),
    ("MI_OFF", 0x7101),
    ("QK_MIDI_TOGGLE", 0x7102),
    ("MI_TOGG", 0x7102),
    ("QK_MIDI_NOTE_C_0", 0x7103),
    ("MI_C", 0x7103),
    ("QK_MIDI_NOTE_C_SHARP_0", 0x7104),
    ("MI_Cs", 0x7104),
    ("MI_Db", 0x7104),
    ("QK_MIDI_NOTE_D_0", 0x7105),
    ("MI_D", 0x7105),
    ("QK_MIDI_NOTE_D_SHARP_0", 0x7106),
    ("MI_Ds", 0x7106),
    ("MI_Eb", 0x7106),
    ("QK_MIDI_NOTE_E_0", 0x7107),
    ("MI_E", 0x7107),
    ("QK_MIDI_NOTE_F_0", 0x7108),
    ("MI_F", 0x7108),
    ("QK_MIDI_NOTE_F_SHARP_0", 0x7109),
    ("MI_Fs", 0x7109),
    ("MI_Gb", 0x7109),
    ("QK_MIDI_NOTE_G_0", 0x710A),
    ("MI_G", 0x710A),
    ("QK_MIDI_NOTE_G_SHARP_0", 0x710B),
    ("MI_Gs", 0x710B),
    ("MI_Ab", 0x710B),
    ("QK_MIDI_NOTE_A_0", 0x710C),
    ("MI_A", 0x710C),
    ("QK_MIDI_NOTE_A_SHARP_0", 0x710D),
    ("MI_As", 0x710D),
    ("MI_Bb", 0x710D),
    ("QK_MIDI_NOTE_B_0", 0x710E),
    ("MI_B", 0x710E),
    ("QK_MIDI_NOTE_C_1", 0x710F),
    ("MI_C1", 0x710F),
    ("QK_MIDI_NOTE_C_SHARP_1", 0x7110),
    ("MI_Cs1", 0x7110),
    ("MI_Db1", 0x7110),
    ("QK_MIDI_NOTE_D_1", 0x7111),
    ("MI_D1", 0x7111),
    ("QK_MIDI_NOTE_D_SHARP_1", 0x7112),
    ("MI_Ds1", 0x7112),
    ("MI_Eb1", 0x7112),
    ("QK_MIDI_NOTE_E_1", 0x7113),
    ("MI_E1", 0x7113),
    ("QK_MIDI_NOTE_F_1", 0x7114),
    ("MI_F1", 0x7114),
    ("QK_MIDI_NOTE_F_SHARP_1", 0x7115),
    ("MI_Fs1", 0x7115),
    ("MI_Gb1", 0x7115),
    ("QK_MIDI_NOTE_G_1", 0x7116),
    ("MI_G1", 0x7116),
    ("QK_MIDI_NOTE_G_SHARP_1", 0x7117),
    ("MI_Gs1", 0x7117),
    ("MI_Ab1", 0x7117),
    ("QK_MIDI_NOTE_A_1", 0x7118),
    ("MI_A1", 0x7118),
    ("QK_MIDI_NOTE_A_SHARP_1", 0x7119),
    ("MI_As1", 0x7119),
    ("MI_Bb1", 0x7119),
    ("QK_MIDI_NOTE_B_1", 0x711A),
    ("MI_B1", 0x711A),
    ("QK_MIDI_NOTE_C_2", 0x711B),
    ("MI_C2", 0x711B),
    ("QK_MIDI_NOTE_C_SHARP_2", 0x711C),
    ("MI_Cs2", 0x711C),
    ("MI_Db2", 0x711C),
    ("QK_MIDI_NOTE_D_2", 0x711D),
    ("MI_D2", 0x711D),
    ("QK_MIDI_NOTE_D_SHARP_2", 0x711E),
    ("MI_Ds2", 0x711E),
    ("MI_Eb2", 0x711E),
    ("QK_MIDI_NOTE_E_2", 0x711F),
    ("MI_E2", 0x711F),
    ("QK_MIDI_NOTE_F_2", 0x7120),
    ("MI_F2", 0x7120),
    ("QK_MIDI_NOTE_F_SHARP_2", 0x7121),
    ("MI_Fs2", 0x7121),
    ("MI_Gb2", 0x7121),
    ("QK_MIDI_NOTE_G_2", 0x7122),
    ("MI_G2", 0x7122),
    ("QK_MIDI_NOTE_G_SHARP_2", 0x7123),
    ("MI_Gs2", 0x7123),
    ("MI_Ab2", 0x7123),
    ("QK_MIDI_NOTE_A_2", 0x7124),
    ("MI_A2", 0x7124),
    ("QK_MIDI_NOTE_A_SHARP_2", 0x7125),
    ("MI_As2", 0x7125),
    ("MI_Bb2", 0x7125),
    ("QK_MIDI_NOTE_B_2", 0x7126),
    ("MI_B2", 0x7126),
    ("QK_MIDI_NOTE_C_3", 0x7127),
    ("MI_C3", 0x7127),
    ("QK_MIDI_NOTE_C_SHARP_3", 0x7128),
    ("MI_Cs3", 0x7128),
    ("MI_Db3", 0x7128),
    ("QK_MIDI_NOTE_D_3", 0x7129),
    ("MI_D3", 0x7129),
    ("QK_MIDI_NOTE_D_SHARP_3", 0x712A),
    ("MI_Ds3", 0x712A),
    ("MI_Eb3", 0x712A),
    ("QK_MIDI_NOTE_E_3", 0x712B),
    ("MI_E3", 0x712B),
    ("QK_MIDI_NOTE_F_3", 0x712C),
    ("MI_F3", 0x712C),
    ("QK_MIDI_NOTE_F_SHARP_3", 0x712D),
    ("MI_Fs3", 0x712D),
    ("MI_Gb3", 0x712D),
    ("QK_MIDI_NOTE_G_3", 0x712E),
    ("MI_G3", 0x712E),
    ("QK_MIDI_NOTE_G_SHARP_3", 0x712F),
    ("MI_Gs3", 0x712F),
    ("MI_Ab3", 0x712F),
    ("QK_MIDI_NOTE_A_3", 0x7130),
    ("MI_A3", 0x7130),
    ("QK_MIDI_NOTE_A_SHARP_3", 0x7131),
    ("MI_As3", 0x7131),
    ("MI_Bb3", 0x7131),
    ("QK_MIDI_NOTE_B_3", 0x7132),
    ("MI_B3", 0x7132),
    ("QK_MIDI_NOTE_C_4", 0x7133),
    ("MI_C4", 0x7133),
    ("QK_MIDI_NOTE_C_SHARP_4", 0x7134),
    ("MI_Cs4", 0x7134),
    ("MI_Db4", 0x7134),
    ("QK_MIDI_NOTE_D_4", 0x7135),
    ("MI_D4", 0x7135),
    ("QK_MIDI_NOTE_D_SHARP_4", 0x7136),
    ("MI_Ds4", 0x7136),
    ("MI_Eb4", 0x7136),
    ("QK_MIDI_NOTE_E_4", 0x7137),
    ("MI_E4", 0x7137),
    ("QK_MIDI_NOTE_F_4", 0x7138),
    ("MI_F4", 0x7138),
    ("QK_MIDI_NOTE_F_SHARP_4", 0x7139),
    ("MI_Fs4", 0x7139),
    ("MI_Gb4", 0x7139),
    ("QK_MIDI_NOTE_G_4", 0x713A),
    ("MI_G4", 0x713A),
    ("QK_MIDI_NOTE_G_SHARP_4", 0x713B),
    ("MI_Gs4", 0x713B),
    ("MI_Ab4", 0x713B),
    ("QK_MIDI_NOTE_A_4", 0x713C),
    ("MI_A4", 0x713C),
    ("QK_MIDI_NOTE_A_SHARP_4", 0x713D),
    ("MI_As4", 0x713D),
    ("MI_Bb4", 0x713D),
    ("QK_MIDI_NOTE_B_4", 0x713E),
    ("MI_B4", 0x713E),
    ("QK_MIDI_NOTE_C_5", 0x713F),
    ("MI_C5", 0x713F),
    ("QK_MIDI_NOTE_C_SHARP_5", 0x7140),
    ("MI_Cs5", 0x7140),
    ("MI_Db5", 0x7140),
    ("QK_MIDI_NOTE_D_5", 0x7141),
    ("MI_D5", 0x7141),
    ("QK_MIDI_NOTE_D_SHARP_5", 0x7142),
    ("MI_Ds5", 0x7142),
    ("MI_Eb5", 0x7142),
    ("QK_MIDI_NOTE_E_5", 0x7143),
    ("MI_E5", 0x7143),
    ("QK_MIDI_NOTE_F_5", 0x7144),
    ("MI_F5", 0x7144),
    ("QK_MIDI_NOTE_F_SHARP_5", 0x7145),
    ("MI_Fs5", 0x7145),
    ("MI_Gb5", 0x7145),
    ("QK_MIDI_NOTE_G_5", 0x7146),
    ("MI_G5", 0x7146),
    ("QK_MIDI_NOTE_G_SHARP_5", 0x7147),
    ("MI_Gs5", 0x7147),
    ("MI_Ab5", 0x7147),
    ("QK_MIDI_NOTE_A_5", 0x7148),
    ("MI_A5", 0x7148),
    ("QK_MIDI_NOTE_A_SHARP_5", 0x7149),
    ("MI_As5", 0x7149),
    ("MI_Bb5", 0x7149),
    ("QK_MIDI_NOTE_B_5", 0x714A),
    ("MI_B5", 0x714A),
    ("QK_MIDI_OCTAVE_N2", 0x714B),
    ("MI_OCN2", 0x714B),
    ("QK_MIDI_OCTAVE_N1", 0x714C),
    ("MI_OCN1", 0x714C),
    ("QK_MIDI_OCTAVE_0", 0x714D),
    ("MI_OC0", 0x714D),
    ("QK_MIDI_OCTAVE_1", 0x714E),
    ("MI_OC1", 0x714E),
    ("QK_MIDI_OCTAVE_2", 0x714F),
    ("MI_OC2", 0x714F),
    ("QK_MIDI_OCTAVE_3", 0x7150),
    ("MI_OC3", 0x7150),
    ("QK_MIDI_OCTAVE_4", 0x7151),
    ("MI_OC4", 0x7151),
    ("QK_MIDI_OCTAVE_5", 0x7152),
    ("MI_OC5", 0x7152),
    ("QK_MIDI_OCTAVE_6", 0x7153),
    ("MI_OC6", 0x7153),
    ("QK_MIDI_OCTAVE_7", 0x7154),
    ("MI_OC7", 0x7154),
    ("QK_MIDI_OCTAVE_DOWN", 0x7155),
    ("MI_OCTD", 0x7155),
    ("QK_MIDI_OCTAVE_UP", 0x7156),
    ("MI_OCTU", 0x7156),
    ("QK_MIDI_TRANSPOSE_N6", 0x7157),
    ("MI_TRN6", 0x7157),
    ("QK_MIDI_TRANSPOSE_N5", 0x7158),
    ("MI_TRN5", 0x7158),
    ("QK_MIDI_TRANSPOSE_N4", 0x7159),
    ("MI_TRN4", 0x7159),
    ("QK_MIDI_TRANSPOSE_N3", 0x715A),
    ("MI_TRN3", 0x715A),
    ("QK_MIDI_TRANSPOSE_N2", 0x715B),
    ("MI_TRN2", 0x715B),
    ("QK_MIDI_TRANSPOSE_N1", 0x715C),
    ("MI_TRN1", 0x715C),
    ("QK_MIDI_TRANSPOSE_0", 0x715D),
    ("MI_TR0", 0x715D),
    ("QK_MIDI_TRANSPOSE_1", 0x715E),
    ("MI_TR1", 0x715E),
    ("QK_MIDI_TRANSPOSE_2", 0x715F),
    ("MI_TR2", 0x715F),
    ("QK_MIDI_TRANSPOSE_3", 0x7160),
    ("MI_TR3", 0x7160),
    ("QK_MIDI_TRANSPOSE_4", 0x7161),
    ("MI_TR4", 0x7161),
    ("QK_MIDI_TRANSPOSE_5", 0x7162),
    ("MI_TR5", 0x7162),
    ("QK_MIDI_TRANSPOSE_6", 0x7163),
    ("MI_TR6", 0x7163),
    ("QK_MIDI_TRANSPOSE_DOWN", 0x7164),
    ("MI_TRSD", 0x7164),
    ("QK_MIDI_TRANSPOSE_UP", 0x7165),
    ("MI_TRSU", 0x7165),
    ("QK_MIDI_VELOCITY_0", 0x7166),
    ("MI_VL0", 0x7166),
    ("QK_MIDI_VELOCITY_1", 0x7167),
    ("MI_VL1", 0x7167),
    ("QK_MIDI_VELOCITY_2", 0x7168),
    ("MI_VL2", 0x7168),
    ("QK_MIDI_VELOCITY_3", 0x7169),
    ("MI_VL3", 0x7169),
    ("QK_MIDI_VELOCITY_4", 0x716A),
    ("MI_VL4", 0x716A),
    ("QK_MIDI_VELOCITY_5", 0x716B),
    ("MI_VL5", 0x716B),
    ("QK_MIDI_VELOCITY_6", 0x716C),
    ("MI_VL6", 0x716C),
    ("QK_MIDI_VELOCITY_7", 0x716D),
    ("MI_VL7", 0x716D),
    ("QK_MIDI_VELOCITY_8", 0x716E),
    ("MI_VL8", 0x716E),
    ("QK_MIDI_VELOCITY_9", 0x716F),
    ("MI_VL9", 0x716F),
    ("QK_MIDI_VELOCITY_10", 0x7170),
    ("MI_VL10", 0x7170),
    ("QK_MIDI_VELOCITY_DOWN", 0x7171),
    ("MI_VELD", 0x7171),
    ("QK_MIDI_VELOCITY_UP", 0x7172),
    ("MI_VELU", 0x7172),
    ("QK_MIDI_CHANNEL_1", 0x7173),
    ("MI_CH1", 0x7173),
    ("QK_MIDI_CHANNEL_2", 0x7174),
    ("MI_CH2", 0x7174),
    ("QK_MIDI_CHANNEL_3", 0x7175),
    ("MI_CH3", 0x7175),
    ("QK_MIDI_CHANNEL_4", 0x7176),
    ("MI_CH4", 0x7176),
    ("QK_MIDI_CHANNEL_5", 0x7177),
    ("MI_CH5", 0x7177),
    ("QK_MIDI_CHANNEL_6", 0x7178),
    ("MI_CH6", 0x7178),
    ("QK_MIDI_CHANNEL_7", 0x7179),
    ("MI_CH7", 0x7179),
    ("QK_MIDI_CHANNEL_8", 0x717A),
    ("MI_CH8", 0x717A),
    ("QK_MIDI_CHANNEL_9", 0x717B),
    ("MI_CH9", 0x717B),
    ("QK_MIDI_CHANNEL_10", 0x717C),
    ("MI_CH10", 0x717C),
    ("QK_MIDI_CHANNEL_11", 0x717D),
    ("MI_CH11", 0x717D),
    ("QK_MIDI_CHANNEL_12", 0x717E),
    ("MI_CH12", 0x717E),
    ("QK_MIDI_CHANNEL_13", 0x717F),
    ("MI_CH13", 0x717F),
    ("QK_MIDI_CHANNEL_14", 0x7180),
    ("MI_CH14", 0x7180),
    ("QK_MIDI_CHANNEL_15", 0x7181),
    ("MI_CH15", 0x7181),
    ("QK_MIDI_CHANNEL_16", 0x7182),
    ("MI_CH16", 0x7182),
    ("QK_MIDI_CHANNEL_DOWN", 0x7183),
    ("MI_CHND", 0x7183),
    ("QK_MIDI_CHANNEL_UP", 0x7184),
    ("MI_CHNU", 0x7184),
    ("QK_MIDI_ALL_NOTES_OFF", 0x7185),
    ("MI_AOFF", 0x7185),
    ("QK_MIDI_SUSTAIN", 0x7186),
    ("MI_SUST", 0x7186),
    ("QK_MIDI_PORTAMENTO", 0x7187),
    ("MI_PORT", 0x7187),
    ("QK_MIDI_SOSTENUTO", 0x7188),
    ("MI_SOST", 0x7188),
    ("QK_MIDI_SOFT", 0x7189),
    ("MI_SOFT", 0x7189),
    ("QK_MIDI_LEGATO", 0x718A),
    ("MI_LEG", 0x718A),
    ("QK_MIDI_MODULATION", 0x718B),
    ("MI_MOD", 0x718B),
    ("QK_MIDI_MODULATION_SPEED_DOWN", 0x718C),
    ("MI_MODD", 0x718C),
    ("QK_MIDI_MODULATION_SPEED_UP", 0x718D),
    ("MI_MODU", 0x718D),
    ("QK_MIDI_PITCH_BEND_DOWN", 0x718E),
    ("MI_BNDD", 0x718E),
    ("QK_MIDI_PITCH_BEND_UP", 0x718F),
    ("MI_BNDU", 0x718F),
    ("QK_SEQUENCER_ON", 0x7200),
    ("SQ_ON", 0x7200),
    ("QK_SEQUENCER_OFF", 0x7201),
    ("SQ_OFF", 0x7201),
    ("QK_SEQUENCER_TOGGLE", 0x7202),
    ("SQ_TOGG", 0x7202),
    ("QK_SEQUENCER_TEMPO_DOWN", 0x7203),
    ("SQ_TMPD", 0x7203),
    ("QK_SEQUENCER_TEMPO_UP", 0x7204),
    ("SQ_TMPU", 0x7204),
    ("QK_SEQUENCER_RESOLUTION_DOWN", 0x7205),
    ("SQ_RESD", 0x7205),
    ("QK_SEQUENCER_RESOLUTION_UP", 0x7206),
    ("SQ_RESU", 0x7206),
    ("QK_SEQUENCER_STEPS_ALL", 0x7207),
    ("SQ_SALL", 0x7207),
    ("QK_SEQUENCER_STEPS_CLEAR", 0x7208),
    ("SQ_SCLR", 0x7208),
    ("QK_JOYSTICK_BUTTON_0", 0x7400),
    ("JS_0", 0x7400),
    ("QK_JOYSTICK_BUTTON_1", 0x7401),
    ("JS_1", 0x7401),
    ("QK_JOYSTICK_BUTTON_2", 0x7402),
    ("JS_2", 0x7402),
    ("QK_JOYSTICK_BUTTON_3", 0x7403),
    ("JS_3", 0x7403),
    ("QK_JOYSTICK_BUTTON_4", 0x7404),
    ("JS_4", 0x7404),
    ("QK_JOYSTICK_BUTTON_5", 0x7405),
    ("JS_5", 0x7405),
    ("QK_JOYSTICK_BUTTON_6", 0x7406),
    ("JS_6", 0x7406),
    ("QK_JOYSTICK_BUTTON_7", 0x7407),
    ("JS_7", 0x7407),
    ("QK_JOYSTICK_BUTTON_8", 0x7408),
    ("JS_8", 0x7408),
    ("QK_JOYSTICK_BUTTON_9", 0x7409),
    ("JS_9", 0x7409),
    ("QK_JOYSTICK_BUTTON_10", 0x740A),
    ("JS_10", 0x740A),
    ("QK_JOYSTICK_BUTTON_11", 0x740B),
    ("JS_11", 0x740B),
    ("QK_JOYSTICK_BUTTON_12", 0x740C),
    ("JS_12", 0x740C),
    ("QK_JOYSTICK_BUTTON_13", 0x740D),
    ("JS_13", 0x740D),
    ("QK_JOYSTICK_BUTTON_14", 0x740E),
    ("JS_14", 0x740E),
    ("QK_JOYSTICK_BUTTON_15", 0x740F),
    ("JS_15", 0x740F),
    ("QK_JOYSTICK_BUTTON_16", 0x7410),
    ("JS_16", 0x7410),
    ("QK_JOYSTICK_BUTTON_17", 0x7411),
    ("JS_17", 0x7411),
    ("QK_JOYSTICK_BUTTON_18", 0x7412),
    ("JS_18", 0x7412),
    ("QK_JOYSTICK_BUTTON_19", 0x7413),
    ("JS_19", 0x7413),
    ("QK_JOYSTICK_BUTTON_20", 0x7414),
    ("JS_20", 0x7414),
    ("QK_JOYSTICK_BUTTON_21", 0x7415),
    ("JS_21", 0x7415),
    ("QK_JOYSTICK_BUTTON_22", 0x7416),
    ("JS_22", 0x7416),
    ("QK_JOYSTICK_BUTTON_23", 0x7417),
    ("JS_23", 0x7417),
    ("QK_JOYSTICK_BUTTON_24", 0x7418),
    ("JS_24", 0x7418),
    ("QK_JOYSTICK_BUTTON_25", 0x7419),
    ("JS_25", 0x7419),
    ("QK_JOYSTICK_BUTTON_26", 0x741A),
    ("JS_26", 0x741A),
    ("QK_JOYSTICK_BUTTON_27", 0x741B),
    ("JS_27", 0x741B),
    ("QK_JOYSTICK_BUTTON_28", 0x741C),
    ("JS_28", 0x741C),
    ("QK_JOYSTICK_BUTTON_29", 0x741D),
    ("JS_29", 0x741D),
    ("QK_JOYSTICK_BUTTON_30", 0x741E),
    ("JS_30", 0x741E),
    ("QK_JOYSTICK_BUTTON_31", 0x741F),
    ("JS_31", 0x741F),
    ("QK_PROGRAMMABLE_BUTTON_1", 0x7440),
    ("PB_1", 0x7440),
    ("QK_PROGRAMMABLE_BUTTON_2", 0x7441),
    ("PB_2", 0x7441),
    ("QK_PROGRAMMABLE_BUTTON_3", 0x7442),
    ("PB_3", 0x7442),
    ("QK_PROGRAMMABLE_BUTTON_4", 0x7443),
    ("PB_4", 0x7443),
    ("QK_PROGRAMMABLE_BUTTON_5", 0x7444),
    ("PB_5", 0x7444),
    ("QK_PROGRAMMABLE_BUTTON_6", 0x7445),
    ("PB_6", 0x7445),
    ("QK_PROGRAMMABLE_BUTTON_7", 0x7446),
    ("PB_7", 0x7446),
    ("QK_PROGRAMMABLE_BUTTON_8", 0x7447),
    ("PB_8", 0x7447),
    ("QK_PROGRAMMABLE_BUTTON_9", 0x7448),
    ("PB_9", 0x7448),
    ("QK_PROGRAMMABLE_BUTTON_10", 0x7449),
    ("PB_10", 0x7449),
    ("QK_PROGRAMMABLE_BUTTON_11", 0x744A),
    ("PB_11", 0x744A),
    ("QK_PROGRAMMABLE_BUTTON_12", 0x744B),
    ("PB_12", 0x744B),
    ("QK_PROGRAMMABLE_BUTTON_13", 0x744C),
    ("PB_13", 0x744C),
    ("QK_PROGRAMMABLE_BUTTON_14", 0x744D),
    ("PB_14", 0x744D),
    ("QK_PROGRAMMABLE_BUTTON_15", 0x744E),
    ("PB_15", 0x744E),
    ("QK_PROGRAMMABLE_BUTTON_16", 0x744F),
    ("PB_16", 0x744F),
    ("QK_PROGRAMMABLE_BUTTON_17", 0x7450),
    ("PB_17", 0x7450),
    ("QK_PROGRAMMABLE_BUTTON_18", 0x7451),
    ("PB_18", 0x7451),
    ("QK_PROGRAMMABLE_BUTTON_19", 0x7452),
    ("PB_19", 0x7452),
    ("QK_PROGRAMMABLE_BUTTON_20", 0x7453),
    ("PB_20", 0x7453),
    ("QK_PROGRAMMABLE_BUTTON_21", 0x7454),
    ("PB_21", 0x7454),
    ("QK_PROGRAMMABLE_BUTTON_22", 0x7455),
    ("PB_22", 0x7455),
    ("QK_PROGRAMMABLE_BUTTON_23", 0x7456),
    ("PB_23", 0x7456),
    ("QK_PROGRAMMABLE_BUTTON_24", 0x7457),
    ("PB_24", 0x7457),
    ("QK_PROGRAMMABLE_BUTTON_25", 0x7458),
    ("PB_25", 0x7458),
    ("QK_PROGRAMMABLE_BUTTON_26", 0x7459),
    ("PB_26", 0x7459),
    ("QK_PROGRAMMABLE_BUTTON_27", 0x745A),
    ("PB_27", 0x745A),
    ("QK_PROGRAMMABLE_BUTTON_28", 0x745B),
    ("PB_28", 0x745B),
    ("QK_PROGRAMMABLE_BUTTON_29", 0x745C),
    ("PB_29", 0x745C),
    ("QK_PROGRAMMABLE_BUTTON_30", 0x745D),
    ("PB_30", 0x745D),
    ("QK_PROGRAMMABLE_BUTTON_31", 0x745E),
    ("PB_31", 0x745E),
    ("QK_PROGRAMMABLE_BUTTON_32", 0x745F),
    ("PB_32", 0x745F),
    ("QK_AUDIO_ON", 0x7480),
    ("AU_ON", 0x7480),
    ("QK_AUDIO_OFF", 0x7481),
    ("AU_OFF", 0x7481),
    ("QK_AUDIO_TOGGLE", 0x7482),
    ("AU_TOGG", 0x7482),
    ("QK_AUDIO_CLICKY_TOGGLE", 0x748A),
    ("CK_TOGG", 0x748A),
    ("QK_AUDIO_CLICKY_ON", 0x748B),
    ("CK_ON", 0x748B),
    ("QK_AUDIO_CLICKY_OFF", 0x748C),
    ("CK_OFF", 0x748C),
    ("QK_AUDIO_CLICKY_UP", 0x748D),
    ("CK_UP", 0x748D),
    ("QK_AUDIO_CLICKY_DOWN", 0x748E),
    ("CK_DOWN", 0x748E),
    ("QK_AUDIO_CLICKY_RESET", 0x748F),
    ("CK_RST", 0x748F),
    ("QK_MUSIC_ON", 0x7490),
    ("MU_ON", 0x7490),
    ("QK_MUSIC_OFF", 0x7491),
    ("MU_OFF", 0x7491),
    ("QK_MUSIC_TOGGLE", 0x7492),
    ("MU_TOGG", 0x7492),
    ("QK_MUSIC_MODE_NEXT", 0x7493),
    ("MU_NEXT", 0x7493),
    ("QK_AUDIO_VOICE_NEXT", 0x7494),
    ("AU_NEXT", 0x7494),
    ("QK_AUDIO_VOICE_PREVIOUS", 0x7495),
    ("AU_PREV", 0x7495),
    ("QK_STENO_BOLT", 0x74F0),
    ("QK_STENO_GEMINI", 0x74F1),
    ("QK_STENO_COMB", 0x74F2),
    ("QK_STENO_COMB_MAX", 0x74FC),
    ("QK_MACRO_0", 0x7700),
    ("MC_0", 0x7700),
    ("QK_MACRO_1", 0x7701),
    ("MC_1", 0x7701),
    ("QK_MACRO_2", 0x7702),
    ("MC_2", 0x7702),
    ("QK_MACRO_3", 0x7703),
    ("MC_3", 0x7703),
    ("QK_MACRO_4", 0x7704),
    ("MC_4", 0x7704),
    ("QK_MACRO_5", 0x7705),
    ("MC_5", 0x7705),
    ("QK_MACRO_6", 0x7706),
    ("MC_6", 0x7706),
    ("QK_MACRO_7", 0x7707),
    ("MC_7", 0x7707),
    ("QK_MACRO_8", 0x7708),
    ("MC_8", 0x7708),
    ("QK_MACRO_9", 0x7709),
    ("MC_9", 0x7709),
    ("QK_MACRO_10", 0x770A),
    ("MC_10", 0x770A),
    ("QK_MACRO_11", 0x770B),
    ("MC_11", 0x770B),
    ("QK_MACRO_12", 0x770C),
    ("MC_12", 0x770C),
    ("QK_MACRO_13", 0x770D),
    ("MC_13", 0x770D),
    ("QK_MACRO_14", 0x770E),
    ("MC_14", 0x770E),
    ("QK_MACRO_15", 0x770F),
    ("MC_15", 0x770F),
    ("QK_MACRO_16", 0x7710),
    ("MC_16", 0x7710),
    ("QK_MACRO_17", 0x7711),
    ("MC_17", 0x7711),
    ("QK_MACRO_18", 0x7712),
    ("MC_18", 0x7712),
    ("QK_MACRO_19", 0x7713),
    ("MC_19", 0x7713),
    ("QK_MACRO_20", 0x7714),
    ("MC_20", 0x7714),
    ("QK_MACRO_21", 0x7715),
    ("MC_21", 0x7715),
    ("QK_MACRO_22", 0x7716),
    ("MC_22", 0x7716),
    ("QK_MACRO_23", 0x7717),
    ("MC_23", 0x7717),
    ("QK_MACRO_24", 0x7718),
    ("MC_24", 0x7718),
    ("QK_MACRO_25", 0x7719),
    ("MC_25", 0x7719),
    ("QK_MACRO_26", 0x771A),
    ("MC_26", 0x771A),
    ("QK_MACRO_27", 0x771B),
    ("MC_27", 0x771B),
    ("QK_MACRO_28", 0x771C),
    ("MC_28", 0x771C),
    ("QK_MACRO_29", 0x771D),
    ("MC_29", 0x771D),
    ("QK_MACRO_30", 0x771E),
    ("MC_30", 0x771E),
    ("QK_MACRO_31", 0x771F),
    ("MC_31", 0x771F),
    ("QK_OUTPUT_AUTO", 0x7780),
    ("OU_AUTO", 0x7780),
    ("QK_OUTPUT_NEXT", 0x7781),
    ("OU_NEXT", 0x7781),
    ("QK_OUTPUT_PREV", 0x7782),
    ("OU_PREV", 0x7782),
    ("QK_OUTPUT_NONE", 0x7783),
    ("OU_NONE", 0x7783),
    ("QK_OUTPUT_USB", 0x7784),
    ("OU_USB", 0x7784),
    ("QK_OUTPUT_2P4GHZ", 0x7785),
    ("OU_2P4G", 0x7785),
    ("QK_OUTPUT_BLUETOOTH", 0x7786),
    ("OU_BT", 0x7786),
    ("QK_BLUETOOTH_PROFILE_NEXT", 0x7790),
    ("BT_NEXT", 0x7790),
    ("QK_BLUETOOTH_PROFILE_PREV", 0x7791),
    ("BT_PREV", 0x7791),
    ("QK_BLUETOOTH_UNPAIR", 0x7792),
    ("BT_UNPR", 0x7792),
    ("QK_BLUETOOTH_PROFILE1", 0x7793),
    ("BT_PRF1", 0x7793),
    ("QK_BLUETOOTH_PROFILE2", 0x7794),
    ("BT_PRF2", 0x7794),
    ("QK_BLUETOOTH_PROFILE3", 0x7795),
    ("BT_PRF3", 0x7795),
    ("QK_BLUETOOTH_PROFILE4", 0x7796),
    ("BT_PRF4", 0x7796),
    ("QK_BLUETOOTH_PROFILE5", 0x7797),
    ("BT_PRF5", 0x7797),
    ("QK_BACKLIGHT_ON", 0x7800),
    ("BL_ON", 0x7800),
    ("QK_BACKLIGHT_OFF", 0x7801),
    ("BL_OFF", 0x7801),
    ("QK_BACKLIGHT_TOGGLE", 0x7802),
    ("BL_TOGG", 0x7802),
    ("QK_BACKLIGHT_DOWN", 0x7803),
    ("BL_DOWN", 0x7803),
    ("QK_BACKLIGHT_UP", 0x7804),
    ("BL_UP", 0x7804),
    ("QK_BACKLIGHT_STEP", 0x7805),
    ("BL_STEP", 0x7805),
    ("QK_BACKLIGHT_TOGGLE_BREATHING", 0x7806),
    ("BL_BRTG", 0x7806),
    ("QK_LED_MATRIX_ON", 0x7810),
    ("LM_ON", 0x7810),
    ("QK_LED_MATRIX_OFF", 0x7811),
    ("LM_OFF", 0x7811),
    ("QK_LED_MATRIX_TOGGLE", 0x7812),
    ("LM_TOGG", 0x7812),
    ("QK_LED_MATRIX_MODE_NEXT", 0x7813),
    ("LM_NEXT", 0x7813),
    ("QK_LED_MATRIX_MODE_PREVIOUS", 0x7814),
    ("LM_PREV", 0x7814),
    ("QK_LED_MATRIX_BRIGHTNESS_UP", 0x7815),
    ("LM_BRIU", 0x7815),
    ("QK_LED_MATRIX_BRIGHTNESS_DOWN", 0x7816),
    ("LM_BRID", 0x7816),
    ("QK_LED_MATRIX_SPEED_UP", 0x7817),
    ("LM_SPDU", 0x7817),
    ("QK_LED_MATRIX_SPEED_DOWN", 0x7818),
    ("LM_SPDD", 0x7818),
    ("QK_UNDERGLOW_TOGGLE", 0x7820),
    ("UG_TOGG", 0x7820),
    ("QK_UNDERGLOW_MODE_NEXT", 0x7821),
    ("UG_NEXT", 0x7821),
    ("QK_UNDERGLOW_MODE_PREVIOUS", 0x7822),
    ("UG_PREV", 0x7822),
    ("QK_UNDERGLOW_HUE_UP", 0x7823),
    ("UG_HUEU", 0x7823),
    ("QK_UNDERGLOW_HUE_DOWN", 0x7824),
    ("UG_HUED", 0x7824),
    ("QK_UNDERGLOW_SATURATION_UP", 0x7825),
    ("UG_SATU", 0x7825),
    ("QK_UNDERGLOW_SATURATION_DOWN", 0x7826),
    ("UG_SATD", 0x7826),
    ("QK_UNDERGLOW_VALUE_UP", 0x7827),
    ("UG_VALU", 0x7827),
    ("QK_UNDERGLOW_VALUE_DOWN", 0x7828),
    ("UG_VALD", 0x7828),
    ("QK_UNDERGLOW_SPEED_UP", 0x7829),
    ("UG_SPDU", 0x7829),
    ("QK_UNDERGLOW_SPEED_DOWN", 0x782A),
    ("UG_SPDD", 0x782A),
    ("RGB_MODE_PLAIN", 0x782B),
    ("RGB_M_P", 0x782B),
    ("RGB_MODE_BREATHE", 0x782C),
    ("RGB_M_B", 0x782C),
    ("RGB_MODE_RAINBOW", 0x782D),
    ("RGB_M_R", 0x782D),
    ("RGB_MODE_SWIRL", 0x782E),
    ("RGB_M_SW", 0x782E),
    ("RGB_MODE_SNAKE", 0x782F),
    ("RGB_M_SN", 0x782F),
    ("RGB_MODE_KNIGHT", 0x7830),
    ("RGB_M_K", 0x7830),
    ("RGB_MODE_XMAS", 0x7831),
    ("RGB_M_X", 0x7831),
    ("RGB_MODE_GRADIENT", 0x7832),
    ("RGB_M_G", 0x7832),
    ("RGB_MODE_RGBTEST", 0x7833),
    ("RGB_M_T", 0x7833),
    ("RGB_MODE_TWINKLE", 0x7834),
    ("RGB_M_TW", 0x7834),
    ("QK_RGB_MATRIX_ON", 0x7840),
    ("RM_ON", 0x7840),
    ("QK_RGB_MATRIX_OFF", 0x7841),
    ("RM_OFF", 0x7841),
    ("QK_RGB_MATRIX_TOGGLE", 0x7842),
    ("RM_TOGG", 0x7842),
    ("QK_RGB_MATRIX_MODE_NEXT", 0x7843),
    ("RM_NEXT", 0x7843),
    ("QK_RGB_MATRIX_MODE_PREVIOUS", 0x7844),
    ("RM_PREV", 0x7844),
    ("QK_RGB_MATRIX_HUE_UP", 0x7845),
    ("RM_HUEU", 0x7845),
    ("QK_RGB_MATRIX_HUE_DOWN", 0x7846),
    ("RM_HUED", 0x7846),
    ("QK_RGB_MATRIX_SATURATION_UP", 0x7847),
    ("RM_SATU", 0x7847),
    ("QK_RGB_MATRIX_SATURATION_DOWN", 0x7848),
    ("RM_SATD", 0x7848),
    ("QK_RGB_MATRIX_VALUE_UP", 0x7849),
    ("RM_VALU", 0x7849),
    ("QK_RGB_MATRIX_VALUE_DOWN", 0x784A),
    ("RM_VALD", 0x784A),
    ("QK_RGB_MATRIX_SPEED_UP", 0x784B),
    ("RM_SPDU", 0x784B),
    ("QK_RGB_MATRIX_SPEED_DOWN", 0x784C),
    ("RM_SPDD", 0x784C),
    ("QK_BOOTLOADER", 0x7C00),
    ("QK_BOOT", 0x7C00),
    ("QK_REBOOT", 0x7C01),
    ("QK_RBT", 0x7C01),
    ("QK_DEBUG_TOGGLE", 0x7C02),
    ("DB_TOGG", 0x7C02),
    ("QK_CLEAR_EEPROM", 0x7C03),
    ("EE_CLR", 0x7C03),
    ("QK_MAKE", 0x7C04),
    ("QK_AUTO_SHIFT_DOWN", 0x7C10),
    ("AS_DOWN", 0x7C10),
    ("QK_AUTO_SHIFT_UP", 0x7C11),
    ("AS_UP", 0x7C11),
    ("QK_AUTO_SHIFT_REPORT", 0x7C12),
    ("AS_RPT", 0x7C12),
    ("QK_AUTO_SHIFT_ON", 0x7C13),
    ("AS_ON", 0x7C13),
    ("QK_AUTO_SHIFT_OFF", 0x7C14),
    ("AS_OFF", 0x7C14),
    ("QK_AUTO_SHIFT_TOGGLE", 0x7C15),
    ("AS_TOGG", 0x7C15),
    ("QK_GRAVE_ESCAPE", 0x7C16),
    ("QK_GESC", 0x7C16),
    ("QK_VELOCIKEY_TOGGLE", 0x7C17),
    ("VK_TOGG", 0x7C17),
    ("QK_SPACE_CADET_LEFT_CTRL_PARENTHESIS_OPEN", 0x7C18),
    ("SC_LCPO", 0x7C18),
    ("QK_SPACE_CADET_RIGHT_CTRL_PARENTHESIS_CLOSE", 0x7C19),
    ("SC_RCPC", 0x7C19),
    ("QK_SPACE_CADET_LEFT_SHIFT_PARENTHESIS_OPEN", 0x7C1A),
    ("SC_LSPO", 0x7C1A),
    ("QK_SPACE_CADET_RIGHT_SHIFT_PARENTHESIS_CLOSE", 0x7C1B),
    ("SC_RSPC", 0x7C1B),
    ("QK_SPACE_CADET_LEFT_ALT_PARENTHESIS_OPEN", 0x7C1C),
    ("SC_LAPO", 0x7C1C),
    ("QK_SPACE_CADET_RIGHT_ALT_PARENTHESIS_CLOSE", 0x7C1D),
    ("SC_RAPC", 0x7C1D),
    ("QK_SPACE_CADET_RIGHT_SHIFT_ENTER", 0x7C1E),
    ("SC_SENT", 0x7C1E),
    ("QK_UNICODE_MODE_NEXT", 0x7C30),
    ("UC_NEXT", 0x7C30),
    ("QK_UNICODE_MODE_PREVIOUS", 0x7C31),
    ("UC_PREV", 0x7C31),
    ("QK_UNICODE_MODE_MACOS", 0x7C32),
    ("UC_MAC", 0x7C32),
    ("QK_UNICODE_MODE_LINUX", 0x7C33),
    ("UC_LINX", 0x7C33),
    ("QK_UNICODE_MODE_WINDOWS", 0x7C34),
    ("UC_WIN", 0x7C34),
    ("QK_UNICODE_MODE_BSD", 0x7C35),
    ("UC_BSD", 0x7C35),
    ("QK_UNICODE_MODE_WINCOMPOSE", 0x7C36),
    ("UC_WINC", 0x7C36),
    ("QK_UNICODE_MODE_EMACS", 0x7C37),
    ("UC_EMAC", 0x7C37),
    ("QK_HAPTIC_ON", 0x7C40),
    ("HF_ON", 0x7C40),
    ("QK_HAPTIC_OFF", 0x7C41),
    ("HF_OFF", 0x7C41),
    ("QK_HAPTIC_TOGGLE", 0x7C42),
    ("HF_TOGG", 0x7C42),
    ("QK_HAPTIC_RESET", 0x7C43),
    ("HF_RST", 0x7C43),
    ("QK_HAPTIC_FEEDBACK_TOGGLE", 0x7C44),
    ("HF_FDBK", 0x7C44),
    ("QK_HAPTIC_BUZZ_TOGGLE", 0x7C45),
    ("HF_BUZZ", 0x7C45),
    ("QK_HAPTIC_MODE_NEXT", 0x7C46),
    ("HF_NEXT", 0x7C46),
    ("QK_HAPTIC_MODE_PREVIOUS", 0x7C47),
    ("HF_PREV", 0x7C47),
    ("QK_HAPTIC_CONTINUOUS_TOGGLE", 0x7C48),
    ("HF_CONT", 0x7C48),
    ("QK_HAPTIC_CONTINUOUS_UP", 0x7C49),
    ("HF_CONU", 0x7C49),
    ("QK_HAPTIC_CONTINUOUS_DOWN", 0x7C4A),
    ("HF_COND", 0x7C4A),
    ("QK_HAPTIC_DWELL_UP", 0x7C4B),
    ("HF_DWLU", 0x7C4B),
    ("QK_HAPTIC_DWELL_DOWN", 0x7C4C),
    ("HF_DWLD", 0x7C4C),
    ("QK_COMBO_ON", 0x7C50),
    ("CM_ON", 0x7C50),
    ("QK_COMBO_OFF", 0x7C51),
    ("CM_OFF", 0x7C51),
    ("QK_COMBO_TOGGLE", 0x7C52),
    ("CM_TOGG", 0x7C52),
    ("QK_DYNAMIC_MACRO_RECORD_START_1", 0x7C53),
    ("DM_REC1", 0x7C53),
    ("QK_DYNAMIC_MACRO_RECORD_START_2", 0x7C54),
    ("DM_REC2", 0x7C54),
    ("QK_DYNAMIC_MACRO_RECORD_STOP", 0x7C55),
    ("DM_RSTP", 0x7C55),
    ("QK_DYNAMIC_MACRO_PLAY_1", 0x7C56),
    ("DM_PLY1", 0x7C56),
    ("QK_DYNAMIC_MACRO_PLAY_2", 0x7C57),
    ("DM_PLY2", 0x7C57),
    ("QK_LEADER", 0x7C58),
    ("QK_LEAD", 0x7C58),
    ("QK_LOCK", 0x7C59),
    ("QK_ONE_SHOT_ON", 0x7C5A),
    ("OS_ON", 0x7C5A),
    ("QK_ONE_SHOT_OFF", 0x7C5B),
    ("OS_OFF", 0x7C5B),
    ("QK_ONE_SHOT_TOGGLE", 0x7C5C),
    ("OS_TOGG", 0x7C5C),
    ("QK_KEY_OVERRIDE_TOGGLE", 0x7C5D),
    ("KO_TOGG", 0x7C5D),
    ("QK_KEY_OVERRIDE_ON", 0x7C5E),
    ("KO_ON", 0x7C5E),
    ("QK_KEY_OVERRIDE_OFF", 0x7C5F),
    ("KO_OFF", 0x7C5F),
    ("QK_SECURE_LOCK", 0x7C60),
    ("SE_LOCK", 0x7C60),
    ("QK_SECURE_UNLOCK", 0x7C61),
    ("SE_UNLK", 0x7C61),
    ("QK_SECURE_TOGGLE", 0x7C62),
    ("SE_TOGG", 0x7C62),
    ("QK_SECURE_REQUEST", 0x7C63),
    ("SE_REQ", 0x7C63),
    ("QK_DYNAMIC_TAPPING_TERM_PRINT", 0x7C70),
    ("DT_PRNT", 0x7C70),
    ("QK_DYNAMIC_TAPPING_TERM_UP", 0x7C71),
    ("DT_UP", 0x7C71),
    ("QK_DYNAMIC_TAPPING_TERM_DOWN", 0x7C72),
    ("DT_DOWN", 0x7C72),
    ("QK_CAPS_WORD_TOGGLE", 0x7C73),
    ("CW_TOGG", 0x7C73),
    ("QK_AUTOCORRECT_ON", 0x7C74),
    ("AC_ON", 0x7C74),
    ("QK_AUTOCORRECT_OFF", 0x7C75),
    ("AC_OFF", 0x7C75),
    ("QK_AUTOCORRECT_TOGGLE", 0x7C76),
    ("AC_TOGG", 0x7C76),
    ("QK_TRI_LAYER_LOWER", 0x7C77),
    ("TL_LOWR", 0x7C77),
    ("QK_TRI_LAYER_UPPER", 0x7C78),
    ("TL_UPPR", 0x7C78),
    ("QK_REPEAT_KEY", 0x7C79),
    ("QK_REP", 0x7C79),
    ("QK_ALT_REPEAT_KEY", 0x7C7A),
    ("QK_AREP", 0x7C7A),
    ("QK_LAYER_LOCK", 0x7C7B),
    ("QK_LLCK", 0x7C7B),
    ("QK_KB_0", 0x7E00),
    ("QK_KB_1", 0x7E01),
    ("QK_KB_2", 0x7E02),
    ("QK_KB_3", 0x7E03),
    ("QK_KB_4", 0x7E04),
    ("QK_KB_5", 0x7E05),
    ("QK_KB_6", 0x7E06),
    ("QK_KB_7", 0x7E07),
    ("QK_KB_8", 0x7E08),
    ("QK_KB_9", 0x7E09),
    ("QK_KB_10", 0x7E0A),
    ("QK_KB_11", 0x7E0B),
    ("QK_KB_12", 0x7E0C),
    ("QK_KB_13", 0x7E0D),
    ("QK_KB_14", 0x7E0E),
    ("QK_KB_15", 0x7E0F),
    ("QK_KB_16", 0x7E10),
    ("QK_KB_17", 0x7E11),
    ("QK_KB_18", 0x7E12),
    ("QK_KB_19", 0x7E13),
    ("QK_KB_20", 0x7E14),
    ("QK_KB_21", 0x7E15),
    ("QK_KB_22", 0x7E16),
    ("QK_KB_23", 0x7E17),
    ("QK_KB_24", 0x7E18),
    ("QK_KB_25", 0x7E19),
    ("QK_KB_26", 0x7E1A),
    ("QK_KB_27", 0x7E1B),
    ("QK_KB_28", 0x7E1C),
    ("QK_KB_29", 0x7E1D),
    ("QK_KB_30", 0x7E1E),
    ("QK_KB_31", 0x7E1F),
    ("QK_USER_0", 0x7E40),
    ("QK_USER_1", 0x7E41),
    ("QK_USER_2", 0x7E42),
    ("QK_USER_3", 0x7E43),
    ("QK_USER_4", 0x7E44),
    ("QK_USER_5", 0x7E45),
    ("QK_USER_6", 0x7E46),
    ("QK_USER_7", 0x7E47),
    ("QK_USER_8", 0x7E48),
    ("QK_USER_9", 0x7E49),
    ("QK_USER_10", 0x7E4A),
    ("QK_USER_11", 0x7E4B),
    ("QK_USER_12", 0x7E4C),
    ("QK_USER_13", 0x7E4D),
    ("QK_USER_14", 0x7E4E),
    ("QK_USER_15", 0x7E4F),
    ("QK_USER_16", 0x7E50),
    ("QK_USER_17", 0x7E51),
    ("QK_USER_18", 0x7E52),
    ("QK_USER_19", 0x7E53),
    ("QK_USER_20", 0x7E54),
    ("QK_USER_21", 0x7E55),
    ("QK_USER_22", 0x7E56),
    ("QK_USER_23", 0x7E57),
    ("QK_USER_24", 0x7E58),
    ("QK_USER_25", 0x7E59),
    ("QK_USER_26", 0x7E5A),
    ("QK_USER_27", 0x7E5B),
    ("QK_USER_28", 0x7E5C),
    ("QK_USER_29", 0x7E5D),
    ("QK_USER_30", 0x7E5E),
    ("QK_USER_31", 0x7E5F),
];

pub const QK_TO: u16 = 0x5200; // TO(n), n in 0..=0x1F
pub const QK_TO_MAX: u16 = 0x001F;
pub const QK_MOMENTARY: u16 = 0x5220; // MO(n), n in 0..=0x1F
pub const QK_MOMENTARY_MAX: u16 = 0x001F;
pub const QK_DEF_LAYER: u16 = 0x5240; // DF(n), n in 0..=0x1F
pub const QK_DEF_LAYER_MAX: u16 = 0x001F;
pub const QK_TOGGLE_LAYER: u16 = 0x5260; // TG(n), n in 0..=0x1F
pub const QK_TOGGLE_LAYER_MAX: u16 = 0x001F;
pub const QK_ONE_SHOT_LAYER: u16 = 0x5280; // OSL(n), n in 0..=0x1F
pub const QK_ONE_SHOT_LAYER_MAX: u16 = 0x001F;
pub const QK_LAYER_TAP_TOGGLE: u16 = 0x52C0; // TT(n), n in 0..=0x1F
pub const QK_LAYER_TAP_TOGGLE_MAX: u16 = 0x001F;
pub const QK_KB_0: u16 = 0x7E00; // HOST(n) custom keycodes, n in 0..=15


/// Legacy names the palette still uses that were renamed upstream in QMK.
const LEGACY: [(&str, &str); 5] = [
    ("KC_NLCK", "KC_NUM_LOCK"),
    ("KC_SLCK", "KC_SCROLL_LOCK"),
    ("KC_PAUS", "KC_PAUSE"),
    ("RESET", "QK_BOOT"),
    ("KC_ENTER", "KC_ENT"),
];

/// C-header defines that exist in quantum_keycodes.h but not in the JSON spec.
/// KC_HYPR = HYPR(KC_NO), KC_MEH = MEH(KC_NO) - modifier masks on no key.
const HEADER_DEFINES: [(&str, u16); 2] = [("KC_HYPR", 0x0F00), ("KC_MEH", 0x0700)];

fn forward_map() -> &'static HashMap<&'static str, u16> {
    static MAP: OnceLock<HashMap<&'static str, u16>> = OnceLock::new();
    MAP.get_or_init(|| {
        let mut m: HashMap<&'static str, u16> = KEYCODES.iter().copied().collect();
        for (old, new) in LEGACY {
            if let Some(&v) = m.get(new) {
                m.entry(old).or_insert(v);
            }
        }
        for (name, val) in HEADER_DEFINES {
            m.entry(name).or_insert(val);
        }
        m
    })
}

fn reverse_map() -> &'static HashMap<u16, &'static str> {
    static MAP: OnceLock<HashMap<u16, &'static str>> = OnceLock::new();
    MAP.get_or_init(|| {
        let mut m = HashMap::new();
        // first occurrence per value wins = the spec's canonical name
        for (name, val) in KEYCODES.iter() {
            m.entry(*val).or_insert(*name);
        }
        m
    })
}

/// Parse "FUNC(n)" -> n, for the given function name.
fn parse_paren(s: &str, func: &str, max: u16) -> Option<u16> {
    let inner = s
        .strip_prefix(func)?
        .strip_prefix('(')?
        .strip_suffix(')')?;
    let n: u16 = inner.trim().parse().ok()?;
    (n <= max).then_some(n)
}

/// QMK keycode string -> wire value. None if unknown.
pub fn name_to_code(name: &str) -> Option<u16> {
    let s = name.trim();
    if let Some(&v) = forward_map().get(s) {
        return Some(v);
    }
    for (func, base, max) in [
        ("MO", QK_MOMENTARY, QK_MOMENTARY_MAX),
        ("TG", QK_TOGGLE_LAYER, QK_TOGGLE_LAYER_MAX),
        ("TO", QK_TO, QK_TO_MAX),
        ("TT", QK_LAYER_TAP_TOGGLE, QK_LAYER_TAP_TOGGLE_MAX),
        ("OSL", QK_ONE_SHOT_LAYER, QK_ONE_SHOT_LAYER_MAX),
        ("DF", QK_DEF_LAYER, QK_DEF_LAYER_MAX),
    ] {
        if let Some(n) = parse_paren(s, func, max) {
            return Some(base | n);
        }
    }
    if let Some(n) = parse_paren(s, "HOST", 15) {
        return Some(QK_KB_0 + n);
    }
    // raw hex escape hatch ("0x7E00")
    if let Some(hex) = s.strip_prefix("0x") {
        return u16::from_str_radix(hex, 16).ok();
    }
    None
}

/// Wire value -> QMK keycode string. Always returns something displayable.
pub fn code_to_name(code: u16) -> String {
    // the app renders these two constantly; keep its preferred spellings
    match code {
        0x0000 => return "KC_NO".into(),
        0x0001 => return "KC_TRNS".into(),
        _ => {}
    }
    for (func, base, max) in [
        ("MO", QK_MOMENTARY, QK_MOMENTARY_MAX),
        ("TG", QK_TOGGLE_LAYER, QK_TOGGLE_LAYER_MAX),
        ("TO", QK_TO, QK_TO_MAX),
        ("TT", QK_LAYER_TAP_TOGGLE, QK_LAYER_TAP_TOGGLE_MAX),
        ("OSL", QK_ONE_SHOT_LAYER, QK_ONE_SHOT_LAYER_MAX),
        ("DF", QK_DEF_LAYER, QK_DEF_LAYER_MAX),
    ] {
        if code >= base && code <= base + max {
            return format!("{}({})", func, code - base);
        }
    }
    if (QK_KB_0..QK_KB_0 + 16).contains(&code) {
        return format!("HOST({})", code - QK_KB_0);
    }
    if let Some(name) = reverse_map().get(&code) {
        return (*name).into();
    }
    format!("0x{code:04X}")
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn basic_roundtrip() {
        for name in ["KC_A", "KC_1", "KC_ESC", "KC_MUTE", "KC_PSLS", "KC_PENT"] {
            let code = name_to_code(name).unwrap_or_else(|| panic!("{name} unknown"));
            assert_eq!(name_to_code(&code_to_name(code)), Some(code), "{name}");
        }
    }

    #[test]
    fn palette_strings_all_resolve() {
        // every string the frontend palette can produce must map to a code
        for name in [
            "KC_NO", "KC_TRNS", "KC_ENTER", "KC_SPACE", "KC_NLCK", "KC_SLCK",
            "KC_PAUS", "RESET", "QK_BOOT", "KC_BRIU", "KC_BRID", "KC_HYPR",
            "KC_MEH", "MO(1)", "MO(3)", "TG(2)", "TO(0)", "OSL(1)", "DF(1)",
            "HOST(0)", "HOST(15)",
        ] {
            assert!(name_to_code(name).is_some(), "{name} failed to resolve");
        }
    }

    #[test]
    fn parameterized_values_match_firmware() {
        assert_eq!(name_to_code("MO(1)"), Some(0x5221));
        assert_eq!(name_to_code("HOST(0)"), Some(0x7E00));
        assert_eq!(name_to_code("HOST(15)"), Some(0x7E0F));
        assert_eq!(code_to_name(0x7E03), "HOST(3)");
        assert_eq!(code_to_name(0x5222), "MO(2)");
    }

    #[test]
    fn unknown_code_formats_as_hex() {
        assert_eq!(code_to_name(0x7FFF), "0x7FFF");
        assert_eq!(name_to_code("0x7FFF"), Some(0x7FFF));
    }
}
